...
class ThugAnalyzer(ThugAPI):
    ...
    def __init__(self, configuration_path="./config/thug", timeout=10, referrer="http://www.google.com"):
...
def run_thug(url, timeout, referrer, config_dir):
    """
    ...
    """
    thug_instance = ThugAnalyzer(config_dir, timeout=timeout, referrer=referrer)
...
def get_args():
    ...
    parser.add_argument("-r", "--referrer", type=str, required=True, help="Referrer")
    parser.add_argument(
        "-t",
        "--timeout",
        default=15,
...
if __name__ == "__main__":
    args = get_args()
    run_thug(args.url, args.timeout, args.referrer, config_dir=args.config_dir)
